# g3_matmul_lz.mojo — G3: the MOJO_CURRICULUM matmul kernels on an Aurora PVC tile.
#
# Kernels (unchanged Mojo source, via Mojo Metal backend -> air2spir -> SPIR-V):
#   naive  : 03a_matmul_naive.mojo   N=2048, 16x16 thread blocks, no shared memory
#   check  : 03d_matmul_check.mojo   N=256,  coarse kernel, FULL CPU-reference check
#   tiled  : 03b_matmul_tiled.mojo   N=2048, 16x16 blocks, shared memory + barrier()
#   coarse : 03c_matmul_coarse.mojo  N=2048, 256-thread blocks, shared memory + barrier()
# N and the block geometry are comptime constants in each kernel; the table in main()
# must match them. Kernel arguments come from `<spv>.args`, written by air2spir.py:
#   buffer -> 8-byte USM holder containing the device data pointer (TileTensor ABI)
#   local  -> shared local memory, bound with a NULL value (zeKernelSetArgumentValue)
#
# Timing methodology is POLARIS/MOJO_CURRICULUM's: warmup launch, ITERS launches, one
# synchronize; GFLOP/s = 2*N^3 / seconds-per-pass. Correctness: 'check' compares every
# element with a CPU triple loop; the N=2048 variants compare SAMPLES (256 entries + corners).
#
# Run all:  qsub g3_run.pbs          Subset:  G3_VARIANTS="naive check" in the environment.
...
def run_variant(mut ctx: IntelGPUContext, v: Variant) raises:
    print("\n=== G3 variant:", v.name, "(", v.src, ") ===")
    var n = v.n
    var bytes = UInt64(n * n * 4)

    # --- argument manifest from air2spir.py ---
...
def main() raises:
    print("=== G3: MOJO_CURRICULUM matmul kernels on an Intel Max 1550 tile ===")
    ...
    table.append(Variant("naive", "03a_matmul_naive.mojo", "g3_naive.spv", 2048, 16, 16, 128, 128, 50, False))
    table.append(Variant("check", "03d_matmul_check.mojo", "g3_check.spv", 256, 256, 1, 2, 2, 5, True))
    table.append(Variant("tiled", "03b_matmul_tiled.mojo", "g3_tiled.spv", 2048, 16, 16, 128, 128, 50, False))
    table.append(Variant("coarse", "03c_matmul_coarse.mojo", "g3_coarse.spv", 2048, 256, 1, 16, 16, 50, False))

    var wanted = getenv("G3_VARIANTS", "naive check tiled coarse")
