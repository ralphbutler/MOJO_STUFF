# run_train_mlp.mojo — G5 step 3: MOJO_CURRICULUM/04b_train_mlp_gpu.mojo training on an Aurora PVC
# tile, with all 12 kernels compiled by OUR spirv64 Mojo backend.
#
# Kernels: emit_mlp.mojo (source-built Mojo, spirv64 target, LLVM SPIR-V codegen) writes
# g5_<role>.spv, .spv.name (entry point) and .spv.args (manifest). No Metal backend, air2spir.py,
# clang or llvm-spirv. The unchanged 04b source; its generic kernels are instantiated with 04b's
# own layouts exactly as 04b's main() does.
#
# Host = ../pre_process/g4_train_mlp_lz.mojo (frozen groundwork) used as a test harness. Differences: the kernel
# files are g5_<role>.spv and the entry-point names come from <spv>.name. The backend's kernel ABI
# passes every argument by pointer, which is exactly G4's binding (8-byte TileTensor holders for
# buffers, 4-byte USM cells for Int32/Float32 scalars). Ordering: ctx.barrier() between the 15
# dependent launches, as in G4 (a Level Zero immediate command list is not in-order).
#
# Correctness bar: 04b's loss curve, 2.17835617 -> 0.000561475754 (NumPy float32 reference),
# printed at epoch 1 and every 50; the stage-by-stage forward check runs first (G4_VERIFY=1).
#
# Run: qsub -v MOJOFILE=run_train_mlp.mojo run_job.pbs
...
struct Launch(Copyable, Movable):
    ...
    var role: String        # which g5_<role>.spv provides the kernel
...
def main() raises:
    print("=== G5: 04b MLP training, kernels from our spirv64 backend, on an Intel Max 1550 tile ===")
    ...
    for entry in table:
        var spv = "g5_" + entry.role + ".spv"
        var manifest = Path(spv + ".args").read_text()
        var kernel_name = Path(spv + ".name").read_text()
        var kernel = Kernel(
