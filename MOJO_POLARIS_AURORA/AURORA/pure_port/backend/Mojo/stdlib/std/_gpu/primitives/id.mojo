...
# AURORA PATCH (G5): Intel GPU (`spirv64`) thread-position builtins. LLVM's
# SPIR-V backend lowers these `llvm.spv.*` intrinsics to OpenCL's vec3 `BuiltIn`
# input variables (`LocalInvocationId`, `WorkgroupId`, `WorkgroupSize`,
# `NumWorkgroups`), indexed by dimension.
@inline(.always)
def _spirv_dim_index[dim: StaticString]() -> Int32:
    comptime if dim == "x":
        return 0
    elif dim == "y":
        return 1
...
@inline(.always)
def _spirv_builtin_id[name: StaticString, dim: StaticString]() -> Int:
    var i = llvm_intrinsic[name, UInt64, has_side_effect=False](
        _spirv_dim_index[dim]()
    )
    return Int(i)
...
struct _ThreadIdx(Defaultable, TrivialRegisterPassable):
    ...
    def __getattr_param__[dim: StringLiteral](self) -> Int:
        ...
        """
        _verify_xyz[dim]()
        comptime if is_intel_gpu():
            return _spirv_builtin_id["llvm.spv.thread.id.in.group.i64", dim]()
        else:
...
struct _BlockIdx(Defaultable, TrivialRegisterPassable):
    ...
    def __getattr_param__[dim: StringLiteral](self) -> Int:
        ...
        _verify_xyz[dim]()
        comptime if is_intel_gpu():
            return _spirv_builtin_id["llvm.spv.group.id.i64", dim]()
        else:
            comptime intrinsic_name = Self._get_intrinsic_name[dim]()
            var i = llvm_intrinsic[
                intrinsic_name, UInt32, has_side_effect=False
...
struct _BlockDim(Defaultable, TrivialRegisterPassable):
    ...
    def __getattr_param__[dim: StaticString](self) -> Int:
        ...
        comptime if is_nvidia_gpu():
            ...
            return _spirv_builtin_id["llvm.spv.workgroup.size.i64", dim]()
        else:
            CompilationTarget.unsupported_target_error[
                operation=__get_current_function_name(),
            ]()
...
struct _GridDim(Defaultable, TrivialRegisterPassable):
    ...
    def __getattr_param__[dim: StaticString](self) -> Int:
        ...
        comptime if is_nvidia_gpu():
            ...
            return _spirv_builtin_id["llvm.spv.num.workgroups.i64", dim]()
        else:
            CompilationTarget.unsupported_target_error[
                operation=__get_current_function_name(),
            ]()
