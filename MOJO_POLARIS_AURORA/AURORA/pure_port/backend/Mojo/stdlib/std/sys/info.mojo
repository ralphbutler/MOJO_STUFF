...
struct Vendor(Equatable, TrivialRegisterPassable, Writable):
    ...
    # AURORA PATCH (G5)
    comptime INTEL_GPU = Self(4)
    """Represents Intel GPU vendor."""
    ...
    def write_to(self, mut writer: Some[Writer]):
        ...
        if self == Vendor.NVIDIA_GPU:
            ...
            return
        if self == Vendor.INTEL_GPU:
            writer.write("intel_gpu")
            return
...
def _vendor_from_arch[arch: StaticString]() -> Vendor:
    ...
    # AURORA PATCH (G5): checked first; "intel-pvc" contains none of the other
    # vendors' substrings, but keep Intel ahead of the loose "mi"/"sm" matches.
    comptime if "intel" in arch:
        return Vendor.INTEL_GPU
    elif "amd" in arch or "gfx" in arch or "mi" in arch:
...
# AURORA PATCH (G5): Intel GPUs through the `spirv64` target.
@inline(.nodebug)
def is_intel_gpu() -> Bool:
    """Returns True if the target triple of the compiler is
    `spirv64-unknown-unknown` (Intel GPU, OpenCL-flavoured SPIR-V) and False
    otherwise.

    Returns:
        True if the triple target is an Intel GPU and False otherwise.
    """
...
def is_gpu() -> Bool:
    ...
    """
    return is_nvidia_gpu() or is_amd_gpu() or is_apple_gpu() or is_intel_gpu()
...
def _macos_version() raises -> Tuple[Int, Int, Int]:
    ...
    var err = external_call["sysctlbyname", Int32](
        "kern.osproductversion".as_c_string_span(),
...
@inline(.nodebug)
def has_intel_gpu_accelerator() -> Bool:
    """Returns True if the host system has an Intel GPU and False otherwise.

    AURORA PATCH (G5).

    Returns:
        True if the host system has an Intel GPU.
    """
    return (
        is_intel_gpu()
        or _vendor_from_arch[_accelerator_arch()]() == Vendor.INTEL_GPU
    )
