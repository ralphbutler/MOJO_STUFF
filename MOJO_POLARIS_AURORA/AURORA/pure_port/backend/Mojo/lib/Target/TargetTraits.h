...
private:
  ...
  // AURORA PATCH (G5): so does the `--target-accelerator` gate, letting an
  // open-source base accelerator target build without MAX installed.
  friend void requireMaxForAcceleratorRequest(llvm::StringRef targetAccelerator);
};
