...
void requireMaxForAcceleratorRequest(llvm::StringRef targetAccelerator) {
  if (targetAccelerator.empty())
    return;
  // AURORA PATCH (G5): an accelerator arch owned by a base (open-source) target
  // does not need MAX.
  if (const TargetTraits *traits = traitsForAcceleratorArch(targetAccelerator);
      traits && traits->isBaseTarget())
    return;
  static const bool maxInstalled = isMaxInstalled();
  if (!maxInstalled) {
