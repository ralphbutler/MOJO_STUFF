...
""")
    ...
    # AURORA PATCH (G5-scope): the hermetic ld64.lld cannot parse the macOS 27 SDK's
    # `arm64e.x1` .tbd stubs; MOJO_MACOS_SDK_PATH pins an older SDK (e.g. MacOSX26.5.sdk).
    sdk_override = rctx.getenv("MOJO_MACOS_SDK_PATH")
    if sdk_override:
        sdk_path = rctx.path(sdk_override)
    else:
