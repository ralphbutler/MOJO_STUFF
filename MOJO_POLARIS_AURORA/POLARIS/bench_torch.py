...
if torch.cuda.is_available():
    # True fp32 (TF32 off) so this is an apples-to-apples match for the fp32 Mojo kernel.
    # A100 TF32 would be much faster but a different numeric format than 03*.
    torch.backends.cuda.matmul.allow_tf32 = False
    avg, gf = bench("cuda")
    print(f"\n--- PyTorch CUDA ({torch.cuda.get_device_name(0)}, cuBLAS, true fp32) ---")
    print(f"  avg time: {avg:.3f} ms")
    print(f"  perf    : {gf:.1f} GFLOP/s")
